...
struct MapView: View {
    ...
    private var searchHistoryView: some View {
        VStack(alignment: .leading, spacing: 0) {
            ForEach(Array(searchHistory.prefix(8).enumerated()), id: \.offset) { index, term in
                historyRow(for: term)
                if index < searchHistory.prefix(8).count - 1 {
                    Divider()
                }
            }
    ...
    private var searchResultsView: some View {
        VStack(alignment: .leading, spacing: 0) {
            ForEach(Array(searchResults.prefix(5).enumerated()), id: \.offset) { index, item in
                resultRow(for: item)
                if index < searchResults.prefix(5).count - 1 {
                    Divider()
                }
            }
        }
    ...
    private func resultRow(for item: MKMapItem) -> some View {
        Button {
    ...
    private func handleSearchResultSelection(_ item: MKMapItem) {
