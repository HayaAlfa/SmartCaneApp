...
struct SavedLocationsView: View {
    ...
    // Provide a default binding so the view can be used without explicitly passing one
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showingAddLocation) {
                // AddLocationView returns SimpleSavedLocation; convert to SavedLocation for this view
                AddLocationView { simple in
                    let converted = SavedLocation(
                        name: simple.name,
                        address: simple.address,
                        latitude: simple.latitude,
                        longitude: simple.longitude,
                        notes: simple.notes,
                        dateAdded: simple.dateAdded
                    )
                    savedLocations.append(converted)
                    saveLocations()
                }
            }
            .onAppear {
        ...
        HStack(spacing: 12) {
            ...
            HStack(spacing: 16) {
                ...
                Button(action: {
                    ...
                    // Switch to Map tab (index 2) and center on this location
                    selectedTab = 2
